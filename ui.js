...
el.innerHTML = `<div class="sessionBannerRow"> <div> <div class="sessionBannerTitle">Session en cours</div> <div id="sessionBannerText" class="sessionBannerText"></div> </div> <button id="btnStopSession" class="btnStop">STOP</button> </div>`;
hero.appendChild(el);
...
return `<button class="statsTab ${isActive ? "statsTab--active" : ""}" onclick="setStatsApp(${id ?`’${id}’`: 'null'})"> ${t.icon} ${t.label} </button>`;
}).join(””);
}
